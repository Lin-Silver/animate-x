...
class FrozenOpenCLIPEmbedder(nn.Module):
    ...
    def encode_with_transformer(self, text):
        ...
        # 修复后的空文本判断逻辑
        if text.shape[0] == 1 and (text == 49406).any() and (text[text != 49406] == 0).all():
            # 空文本，使用 1x1 的 attn_mask
            attn_mask = torch.ones(1, 1, device=self.device)
        else:
            # 计算有效 token 的最大长度（忽略填充的零）
            seq_len = (text != 0).sum(dim=-1).max().item()
            attn_mask = self.model.attn_mask[:seq_len, :seq_len] if self.model.attn_mask is not None else None
...
class FrozenOpenCLIPTextVisualEmbedder(nn.Module):
    ...
    def encode_with_transformer(self, text):
        ...
        # 修改后的 attn_mask 生成逻辑
        if text.shape[0] == 1 and (text == 49406).any():  # 检查是否存在 [SOS] token
            # 空文本，使用 1x1 的掩码
            attn_mask = torch.ones(1, 1, device=self.device)
        else:
            # 计算实际有效 token 的最大长度（忽略填充的零）
            seq_len = (text != 0).sum(dim=-1).max().item()
            attn_mask = self.model.attn_mask[:seq_len, :seq_len] if self.model.attn_mask is not None else None
